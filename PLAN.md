...
- Keep `~/firstmate/projects` as the Linux project root. The intended Windows client command is the configured `herdr` function, which uses the `wsl-herdr` SSH-config alias.
...
- Windows installation instructions use the official Herdr PowerShell installer, and remote attach is documented through the configured `herdr` function and `wsl-herdr` SSH-config alias.
...
|   |-- agy-query.sh                     # concise AGY terminal Q&A CLI (? alias)
...
`-- tests/
    ...
    |-- test-agy-query.sh                 # AGY Q&A deterministic tests
